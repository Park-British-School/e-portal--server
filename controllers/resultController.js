...
exports.getResult = async function (req, res) {
  const result = await Result.findById(req.params.id).populate(["class", "student", "uploadedBy"])
...
exports.downloadResult = async function (req, res, next) {
  try {
    const resultID = req.params.resultID
    const result = await Result.findById(resultID).populate(["class", "student", "uploadedBy"])
    if(!result) {
