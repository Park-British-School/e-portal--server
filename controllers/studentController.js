...
exports.getStudent = async function (req, res) {
  ...
  const student = await Student.findById(studentID).populate(['class', "results"])
...
exports.login = async function (req, res) {
  ...
  try {
    ...
    const student = await Student.findOne({ _id: req.body.id }).populate(["class", "results"])
    if (student) {
      ...
      if (isPasswordMatched) {
        ...
        if (isActive) {
          await Student.updateOne({ _id: student._id }, { lastSeen: new Date().getTime() })
...
exports.getStudents = async function (req, res) {
  try {
    ...
    const students = req.body.find({ id: studentID }).populate(['class', "results"])
